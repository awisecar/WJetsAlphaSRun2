...
if ( $withoptional ) then
    ...
    #tar xzf ${arc}.tar.gz ## for hoppet-1.2.0
    cd ${arc}
...
# fastNLO Toolkit:
#------------------------------------------------------------------------------
set arc="fastnlo_toolkit-2.3.1pre-2441" ##one that works with current version of MCgrid
#set arc="fastnlo_toolkit-2.3.1-2657" ##one that reads scale variations using hoppet v1.2.0
echo ""
echo "$arc"
echo ""
if ( ! -e ${arc}_installed  ) then
...
# One can add other processes later on
        ...
        ## Install the W, W+jets libraries -- AW
...
#------------------------------------------------------------------------------
    ...
    if ( ! -e ${arc}_installed  ) then
        ...
        cd ${arc}
        #./configure --prefix=${base}/${local} --with-sqlite3=install --enable-gzip --enable-lhole --enable-fastjet=${base}/${local} --enable-lhapdf=${base}/${local} --enable-hepmc2=${base}/${local} --enable-rivet=${base}/${local} ${rootenablepath} --enable-openloops=${base}/FastNLOInstall/OpenLoops-1.3.1 ${mpiopt} --enable-blackhat=${base}/${local}
        ./configure --prefix=${base}/${local} --with-sqlite3=install --enable-gzip --enable-lhole --enable-fastjet=${base}/${local} --enable-lhapdf=${base}/${local} --enable-hepmc2=${base}/${local} --enable-rivet=${base}/${local} ${rootenablepath} --enable-openloops=${base}/FastNLOInstallv3/OpenLoops-2.0.0 ${mpiopt} --enable-blackhat=${base}/${local}
        make -j${cores} install
        cd ..
        touch ${arc}_installed
    endif
...
#    endif 
#
endif ##end of the (withsherpa) requirement
#
